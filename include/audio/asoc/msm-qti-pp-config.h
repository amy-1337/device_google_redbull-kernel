...
#else /* CONFIG_QTI_PP */
static __inline__ int msm_adsp_inform_mixer_ctl(struct snd_soc_pcm_runtime *rtd,
			uint32_t *payload)
{
	return 0;
}

static __inline__ int msm_adsp_init_mixer_ctl_pp_event_queue(
			struct snd_soc_pcm_runtime *rtd)
{
	return 0;
}

static __inline__ int msm_adsp_clean_mixer_ctl_pp_event_queue(
			struct snd_soc_pcm_runtime *rtd)
{
	return 0;
}

static __inline__ int msm_adsp_stream_cmd_info(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_info *uinfo)
{
	return 0;
}

static __inline__ int msm_adsp_stream_callback_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static __inline__ int msm_adsp_stream_callback_info(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_info *uinfo)
{
	return 0;
}
