...
#else
static __inline__ int aqt_mbhc_init(struct aqt1000_mbhc **mbhc,
				  struct snd_soc_component *component,
				  struct fw_info *fw_data)
{
	return 0;
}
static __inline__ void aqt_mbhc_hs_detect_exit(struct snd_soc_component *component)
{
}
static __inline__ int aqt_mbhc_hs_detect(struct snd_soc_component *component,
				       struct wcd_mbhc_config *mbhc_cfg)
{
		return 0;
}
static __inline__ void aqt_mbhc_deinit(struct snd_soc_component *component)
{
}
static __inline__ int aqt_mbhc_post_ssr_init(struct aqt1000_mbhc *mbhc,
					   struct snd_soc_component *component)
{
	return 0;
}

static __inline__ int aqt_mbhc_get_impedance(struct aqt1000_mbhc *aqt_mbhc,
					   uint32_t *zl, uint32_t *zr)
{
	if (zl)
		*zl = 0;
