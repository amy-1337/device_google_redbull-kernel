...
struct sde_rotation_item {
	...
	/* Completion structure for __inline__ rotation */
...
static __inline__ int sde_rotator_is_valid_pixfmt(struct sde_rot_mgr *mgr,
...
static __inline__ u32 sde_rotator_get_pixfmt(struct sde_rot_mgr *mgr,
...
static __inline__ int sde_rotator_get_downscale_caps(struct sde_rot_mgr *mgr,
		char *caps, int len)
{
...
static __inline__ int sde_rotator_get_maxlinewidth(struct sde_rot_mgr *mgr)
{
...
static __inline__ int __compare_session_item_rect(
 ...
 */
static __inline__ int __compare_session_rotations(uint32_t cfg_flag,
	uint32_t item_flag)
{
...
/*
 * sde_rotator_req_reset_start - reset __inline__ h/w 'start' indicator
 *	For __inline__ rotations, the time of rotation start is not controlled
 *	by the rotator driver. This function resets an internal 'start'
 *	indicator that allows the rotator to delay its rotator
 *	timeout waiting until such time as the __inline__ rotation has
 *	really started.
 * @mgr: Pointer to rotator manager
...
/*
 * sde_rotator_req_set_start - set __inline__ h/w 'start' indicator
 * @mgr: Pointer to rotator manager
 * @req: Pointer to rotation request
 */
...
/*
 * sde_rotator_req_wait_start - wait for __inline__ h/w 'start' indicator
 * @mgr: Pointer to rotator manager
 * @req: Pointer to rotation request
...
/*
 * sde_rotator_abort_inline_request - abort __inline__ rotation request after start
 *	This function allows __inline__ rotation requests to be aborted after
 ...
 */
static __inline__ int sde_rotator_resource_ctrl_enabled(struct sde_rot_mgr *mgr)
{
 ...
 * @mgr: Pointer to rotator manager
 */
static __inline__ void sde_rot_mgr_lock(struct sde_rot_mgr *mgr)
{
	mutex_lock(&mgr->lock);
}
 ...
 * @mgr: Pointer to rotator manager
 */
static __inline__ void sde_rot_mgr_unlock(struct sde_rot_mgr *mgr)
{
 ...
 * @mgr: Pointer to rotator manager
 */
static __inline__ bool sde_rot_mgr_pd_enabled(struct sde_rot_mgr *mgr)
{
