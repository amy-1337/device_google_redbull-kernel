...
static __inline__ void msm_kms_init(struct msm_kms *kms,
		const struct msm_kms_funcs *funcs)
{
	kms->funcs = funcs;
}
...
#else
static __inline__ int msm_mdss_init(struct drm_device *dev)
{
	return 0;
}
static __inline__ void msm_mdss_destroy(struct drm_device *dev)
{
}
static __inline__ struct msm_kms *mdp5_kms_init(struct drm_device *dev)
{
	return NULL;
}
static __inline__ int msm_mdss_enable(struct msm_mdss *mdss)
{
	return 0;
}
static __inline__ int msm_mdss_disable(struct msm_mdss *mdss)
{
	return 0;
}
#endif
 ...
 */
static __inline__ bool msm_is_mode_seamless(const struct drm_display_mode *mode)
{
	return (mode->flags & DRM_MODE_FLAG_SEAMLESS);
}

static __inline__ bool msm_is_mode_seamless_dms(const struct drm_display_mode *mode)
{
...
static __inline__ bool msm_is_mode_dynamic_fps(const struct drm_display_mode *mode)
{
	return ((mode->flags & DRM_MODE_FLAG_SEAMLESS) &&
		(mode->private_flags & MSM_MODE_FLAG_SEAMLESS_DYNAMIC_FPS));
}

static __inline__ bool msm_is_mode_seamless_vrr(const struct drm_display_mode *mode)
{
	return mode ? (mode->private_flags & MSM_MODE_FLAG_SEAMLESS_VRR)
		: false;
}

static __inline__ bool msm_is_mode_seamless_poms(
		const struct drm_display_mode *mode)
{
...
static __inline__ bool msm_is_mode_seamless_dyn_clk(
					const struct drm_display_mode *mode)
{
...
static __inline__ bool msm_needs_vblank_pre_modeset(
		const struct drm_display_mode *mode)
{
