...
struct sde_kms {
	...
	/* io/register spaces: */
	void *mmio, *vbif[VBIF_MAX], *reg_dma, *sid,
		*imem;
 ...
 */
static __inline__ bool sde_kms_power_resource_is_enabled(struct drm_device *dev)
{
	if (!dev)
		return false;
 ...
 */
static __inline__ bool sde_kms_is_suspend_state(struct drm_device *dev)
{
 ...
 */
static __inline__ bool sde_kms_is_suspend_blocked(struct drm_device *dev)
{
 ...
 */
static __inline__ bool sde_kms_is_secure_session_inprogress(struct sde_kms *sde_kms)
{
	bool ret = false;
 ...
 */
static __inline__ bool sde_kms_is_vbif_operation_allowed(struct sde_kms *sde_kms)
{
	if (!sde_kms)
		return false;
 ...
 * @sde_kms: Pointer to sde_kms
 */
static __inline__ bool sde_kms_is_cp_operation_allowed(struct sde_kms *sde_kms)
{
 ...
 */
static __inline__ bool sde_kms_rect_is_equal(struct sde_rect *r1,
		struct sde_rect *r2)
{
	if ((!r1 && r2) || (r1 && !r2))
		return false;
 ...
 */
static __inline__ bool sde_kms_rect_is_null(const struct sde_rect *r)
{
	if (!r)
		return true;
