...
static __inline__ struct swr_master *to_swr_master(struct device *dev)
{
...
static __inline__ struct swr_device *to_swr_device(struct device *dev)
{
...
static __inline__ struct swr_driver *to_swr_driver(struct device_driver *drv)
{
...
static __inline__ void *swr_get_ctrl_data(const struct swr_master *master)
{
	return master ? dev_get_drvdata(&master->dev) : NULL;
}

static __inline__ void swr_set_ctrl_data(struct swr_master *master, void *data)
{
	dev_set_drvdata(&master->dev, data);
}

static __inline__ void *swr_get_dev_data(const struct swr_device *dev)
{
	return dev ? dev_get_drvdata(&dev->dev) : NULL;
}

static __inline__ void swr_set_dev_data(struct swr_device *dev, void *data)
{
