...
static __inline__ int msm_sdw_set_spkr_mode(struct snd_soc_component *component,
					int mode)
{
	return 0;
}
static __inline__ int msm_sdw_set_spkr_gain_offset(
				struct snd_soc_component *component,
				int offset);
{
	return 0;
}
static __inline__ void msm_sdw_gpio_cb(
	int (*sdw_cdc_gpio_fn)(bool enable,
	struct snd_soc_component *component),
	struct snd_soc_component *component);
{
...
static __inline__ struct regmap *msm_sdw_regmap_init(struct device *dev,
					  const struct regmap_config *config);
{
	return NULL;
}
static __inline__ int msm_sdw_codec_info_create_codec_entry(
	struct snd_info_entry *component_root,
	struct snd_soc_component *component)
{
	return 0;
