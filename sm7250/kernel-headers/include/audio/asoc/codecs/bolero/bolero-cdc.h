...
#else
static __inline__ int bolero_register_res_clk(struct device *dev, rsc_clk_cb_t cb)
{
	return 0;
}
static __inline__ void bolero_unregister_res_clk(struct device *dev)
{
}
...
static __inline__ int bolero_register_macro(struct device *dev,
					u16 macro_id,
					struct macro_ops *ops)
{
	return 0;
...
static __inline__ void bolero_unregister_macro(struct device *dev, u16 macro_id)
{
}

static __inline__ struct device *bolero_get_device_ptr(struct device *dev,
						   u16 macro_id)
{
	return NULL;
}
...
static __inline__ void bolero_clear_amic_tx_hold(struct device *dev, u16 adc_n)
{
}

static __inline__ int bolero_register_wake_irq(struct snd_soc_component *component,
					   u32 data)
{
	return 0;
}

static __inline__ int bolero_runtime_resume(struct device *dev)
{
	return 0;
}
...
static __inline__ int bolero_set_port_map(struct snd_soc_component *component,
				u32 size, void *data)
{
	return 0;
}

static __inline__ int bolero_tx_clk_switch(struct snd_soc_component *component,
					int clk_src)
{
	return 0;
}

static __inline__ int bolero_register_event_listener(
					struct snd_soc_component *component,
					bool enable)
{
	return 0;
...
static __inline__ bool bolero_check_core_votes(struct device *dev)
{
	return false;
}
