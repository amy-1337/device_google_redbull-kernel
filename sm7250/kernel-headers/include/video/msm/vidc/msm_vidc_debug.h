...
static __inline__ char *get_debug_level_str(int level)
{
	switch (level) {
	case VIDC_ERR:
		return "err ";
 ...
 */
static __inline__ bool is_print_allowed(u32 sid, u32 level)
{
...
static __inline__ char *get_codec_name(u32 sid)
{
	if (!sid || sid > MAX_SUPPORTED_INSTANCES)
		return ".....";
...
static __inline__ void put_sid(u32 sid)
{
	if (!sid || sid > MAX_SUPPORTED_INSTANCES) {
...
static __inline__ void tic(struct msm_vidc_inst *i, enum profiling_points p,
				 char *b)
{
...
static __inline__ void toc(struct msm_vidc_inst *i, enum profiling_points p)
{
	struct timeval __ddl_tv;
...
static __inline__ void show_stats(struct msm_vidc_inst *i)
{
	int x;
...
static __inline__ void msm_vidc_res_handle_fatal_hw_error(
...
static __inline__ void msm_vidc_handle_hw_error(struct msm_vidc_core *core)
{
